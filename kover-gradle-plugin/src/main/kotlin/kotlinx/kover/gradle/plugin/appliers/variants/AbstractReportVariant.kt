...
import kotlinx.kover.gradle.plugin.sources.VariantSource
...
internal sealed class AbstractReportVariant(
    ...
    protected fun fromSource(variantSource: VariantSource, compilationFilter: (String) -> Boolean = { true }) {
        val tests = variantSource.tests.matching {
            // skip all tests from instrumentation if Kover Plugin is disabled for the project
            !variantConfig.instrumentation.disableAll.get()
        ...
        val compilations = variantSource.compilations.map { it.filterKeys(compilationFilter).values }
